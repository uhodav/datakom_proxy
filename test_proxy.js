...
// Альтернативный резервный список SOCKS5-прокси (можно расширить)
...
async function loadProxies() {
  try {
    ...
    console.log('Список с основного источника пуст, используем резервный.');
    return fallbackProxies;
  } catch (e) {
    console.log('Ошибка загрузки основного списка, используем резервный:', e.message);
    return fallbackProxies;
  }
}
...
async function testProxy(proxy) {
  ...
  try {
    // Получаем свой IP через сервис, который возвращает IP клиента
    ...
    // Проверяем доступность google.com
...
(async () => {
  ...
  for (const proxy of proxies) {
    ...
    if (ok) break; // Остановиться на первом рабочем
  }
})();
